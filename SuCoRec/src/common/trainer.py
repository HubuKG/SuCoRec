...
class Trainer(AbstractTrainer):
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if verbose:
                self.logger.info(
                    f"Progress: {progress * 100:.2f}%, Elapsed Time: {elapsed_time:.2f}s, Estimated Remaining Time: {remaining_time:.2f}s")
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if not hasattr(self, 'best_metric'):
                    self.best_metric = valid_score
                    self.previous_metric = valid_score


                if valid_score > self.best_metric:
                    self.model.local_sample_p = min(self.model.local_sample_p + self.model.p_step, self.model.p_max)
                    self.best_metric = valid_score
                    self.logger.info(
                        f"Validation metric improved to {valid_score:.4f}, increasing local_sample_p to {self.model.local_sample_p:.3f}")
                    self.no_improve_cnt = 0
                else:
                    self.model.local_sample_p = max(self.model.local_sample_p - self.model.p_step, self.model.p_min)
                    self.no_improve_cnt += 1
                    self.logger.info(
                        f"Validation metric did not improve (current {valid_score:.4f}), decreasing local_sample_p to {self.model.local_sample_p:.3f}, no improvement for {self.no_improve_cnt} consecutive times")
